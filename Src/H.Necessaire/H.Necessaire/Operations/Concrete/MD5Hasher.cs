...
{
    public class MD5Hasher : ImAHasherEngine
    {
        #region Construct
        /*
         * Round shift values
        ...
        {
            string md5Hash = CalculateMD5ForBytes(Encoding.UTF8.GetBytes(value));
        ...
        public Task<OperationResult> VerifyMatch(string value, SecuredHash againstHashedValue)
        {
            string md5Hash = CalculateMD5ForBytes(Encoding.UTF8.GetBytes(value));
            ...
            return
                new OperationResult
                {
                    IsSuccessful = md5Hash == againstHashedValue?.Hash,
                }
                .AsTask()
                ;
        }
        ...
        private static string CalculateMD5ForBytes(byte[] input)
        {
