...
namespace H.Necessaire.Runtime.RavenDB
{
    public static class RavenQueryExtensions
    {
        ...
        public static IDocumentQuery<TEntity> ApplySyncRavenDbSortAndPageFilterIfAny<TEntity, TFilter>(this IDocumentQuery<TEntity> ravenQueryable, TFilter filter, bool throwOnValidationError = true)
            where TFilter : ISortFilter, IPageFilter
        {
            if (filter?.SortFilters?.Any() ?? false)
            {
